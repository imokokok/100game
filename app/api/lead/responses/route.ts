...
export async function GET(req:NextRequest){if(!isOwner(req)&&!await isLead(req))return NextResponse.json({error:"Unauthorized"},{status:401,headers:{"cache-control":"no-store"}});const {results}=await d1().prepare("SELECT id, wechat_name, locale, payload, submitted_at FROM questionnaire_responses ORDER BY submitted_at DESC LIMIT 1000").all();return NextResponse.json({responses:results},{headers:{"cache-control":"private, no-store","x-content-type-options":"nosniff"}})}
