...
import { canAccessGroup, d1, isLead, participantId, getBlob, putBlob, deleteBlob } from "../_shared";
...
export async function POST(req: NextRequest) {
  ...
  if (!participant && !lead) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  ...
  const ownerKey = lead ? "owner" : participant!;
  ...
  const objectUrl = await putBlob(objectKey, file, file.type);
  const now = Date.now();
  try{
    await d1().prepare("INSERT INTO uploaded_files (id, participant_id, group_id, task_id, category, week, object_key, name, content_type, size, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(id, ownerKey, groupId, taskId, category, week, objectUrl, file.name.slice(0, 240), file.type, file.size, now).run();
  }catch(error){
    await deleteBlob(objectUrl).catch(()=>undefined);
    throw error;
  }
...
export async function GET(req: NextRequest) {
  ...
  if (!id && !groupId && !category && req.nextUrl.searchParams.has("week")) {
    if (!(await participantId(req)) && !(await isLead(req))) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  ...
  const allowed = await isLead(req) || (row.group_id ? await canAccessGroup(req, row.group_id) : workspaceCategories.has(row.category) ? Boolean(viewer) : viewer === row.participant_id);
