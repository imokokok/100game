...
export async function POST(req: NextRequest) {
  if (!(await isLead(req))) return NextResponse.json({ error: "Only the Lead Designer can create invitations" }, { status: 403 });
...
export async function PATCH(req: NextRequest) {
  if (!(await isLead(req))) return NextResponse.json({ error: "Only the Lead Designer can revoke invitations" }, { status: 403 });
  const body = await req.json().catch(() => ({}));
