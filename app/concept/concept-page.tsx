...
function PublicDesignerRanking({lang}:{lang:Language}){
 ...
 return <section className="publicCredits publicCreditsPending" id="designers"><div className="creditsHead" data-reveal data-motion="people-head"><span className="kicker">{c.collaborators}</span><h1 id="public-view-title" tabIndex={-1} className="publicBrandTitle"><PublicTitlePicture priority/></h1><p>{lang==="zh"?"共同创作者名单将在人员信息确认后统一录入。":"The collaborator list will be added after participant information is confirmed."}</p></div><div className="collaboratorPending" role="status" data-reveal data-motion="people-list"><span>01—100</span><b>{lang==="zh"?"待录入":"To be added"}</b></div></section>
}
...
export function PublicPages({initialView}:{initialView:PublicView}){
 ...
 useEffect(()=>{const page=document.querySelector<HTMLElement>(view==="concept"?".conceptPage":".peoplePage");if(!page)return;const nodes=Array.from(page.querySelectorAll<HTMLElement>("[data-reveal]"));page.classList.add("motion-ready");nodes.forEach((node,index)=>node.style.setProperty("--reveal-index",String(index)));if(!("IntersectionObserver" in window)){nodes.forEach(node=>node.classList.add("is-visible"));return}const observer=new IntersectionObserver(entries=>entries.forEach(entry=>{if(entry.isIntersecting){(entry.target as HTMLElement).classList.add("is-visible");observer.unobserve(entry.target)}}),{rootMargin:"0px 0px -7% 0px",threshold:.08});nodes.forEach(node=>observer.observe(node));return()=>observer.disconnect()},[view,lang]);
 ...
 const go=(next:PublicView)=>(event:MouseEvent<HTMLAnchorElement>)=>{if(event.button!==0||event.metaKey||event.ctrlKey||event.shiftKey||event.altKey)return;event.preventDefault();if(view===next)return;history.pushState({},"",next==="concept"?"/concept":"/people");setView(next);window.scrollTo(0,0);window.setTimeout(()=>document.getElementById("public-view-title")?.focus({preventScroll:true}),0)};
 ...
 return <main className={`publicPage ${view==="concept"?"conceptPage":"peoplePage"} ${compactManifesto?"manifesto-ready":""}`}><header className="top"><a className="publicBack" href="/">← {c.back}</a><a className="mark" href="/" aria-label="WHAT 100 PEOPLE DO TO A GAME"><span className="fullPublicMark"><Wordmark/></span><span className="compactPublicMark" aria-hidden="true">WHAT 100 / GAME</span></a><nav aria-label={c.publicPages}><a className={view==="concept"?"active":""} aria-current={view==="concept"?"page":undefined} href="/concept" onClick={go("concept")}>{c.concept}</a><a className={view==="people"?"active":""} aria-current={view==="people"?"page":undefined} href="/people" onClick={go("people")}>{c.people}</a></nav><label className="languagePicker"><span><b aria-hidden="true">🌐</b>{c.language}</span><select aria-label={c.language} value={lang} onChange={e=>{const next=e.target.value as Language;storageSet("hundred-language",next);setLang(next)}}>{languageOptions.filter(x=>x.value==="zh"||x.value==="en").map(x=><option key={x.value} value={x.value}>{x.label}</option>)}</select></label></header>{view==="concept"?<section className="conceptStandalone"><header className="conceptHero"><h1 id="public-view-title" tabIndex={-1} className="publicBrandTitle" data-reveal data-motion="title"><PublicTitlePicture priority/></h1><p className="conceptSubtitle" data-reveal data-motion="subtitle">{editorial?editorial.subtitle:c.subtitle}</p>{editorial&&<div className="conceptQuestions" data-reveal data-motion="questions"><span>QUESTIONS FOR THE MEDIUM</span>{editorial.questions.map(question=><p key={question}>{question}</p>)}</div>}<ProjectMetrics lang={lang} open={metricPanel} onToggle={panel=>setMetricPanel(current=>current===panel?null:panel)} links={gameLinks} linksLoaded={gameLinksLoaded}/></header><section className="conceptFeature" data-reveal data-motion="feature"><div className="conceptFeatureCopy"><span>PROJECT 01 / OPEN CALL</span><h2>{lang==="zh"?"我们不寻找一百名游戏设计师。":"We are not looking for one hundred game designers."}</h2><p><span>{editorial?editorial.essay[3]:c.essay[0]}</span></p><strong>{lang==="zh"?"我们寻找一百个人。":"We are looking for one hundred people."}</strong></div></section><section className="conceptManifesto" aria-label={lang==="zh"?"项目理念":"Project manifesto"}>{manifesto.map((item,index)=>{const expanded=!compactManifesto||openManifesto===index;return <article key={item.code} className={openManifesto===index?"is-open":""} data-reveal data-motion={index%2===0?"manifesto-left":"manifesto-right"}><button type="button" className="manifestoToggle" aria-expanded={expanded} aria-controls={`manifesto-body-${index}`} aria-disabled={!compactManifesto} tabIndex={compactManifesto?0:-1} onClick={()=>{if(compactManifesto)setOpenManifesto(current=>current===index?-1:index)}}><span className="manifestoCode">{item.code}</span><h2>{item.title}</h2><i aria-hidden="true"/></button><div id={`manifesto-body-${index}`} className="manifestoBody" aria-hidden={!expanded} inert={!expanded?true:undefined}><div><p>{item.body}</p></div></div></article>})}</section>{editorial&&<aside className="conceptRecruitment" data-reveal data-motion="recruitment"><span className="kicker">{editorial.call.kicker}</span><h2>{editorial.call.title}</h2><p>{editorial.call.body}</p><strong>{editorial.call.deadline}</strong><p>{editorial.call.contact}</p><b>{editorial.call.action}</b></aside>}<a className="peopleCta" href="/people" onClick={go("people")} data-reveal data-motion="cta">{c.meet} →</a><RecruitmentPoster lang={lang}/></section>:<PublicDesignerRanking lang={lang}/>}<footer className="copyright">{t.copyright}</footer></main>
}
