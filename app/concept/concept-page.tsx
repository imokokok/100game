...
export function PublicPages({initialView}:{initialView:PublicView}){
 ...
 const [openManifesto,setOpenManifesto]=useState(0);
 ...
 const manifesto=[
  ...
  {code:"02 / RESOURCES",title:lang==="zh"?"重新分配进入、表达与决定的权利":"Redistributing the rights to enter, speak and decide",body:editorial?editorial.essay[4]:c.essay[1]},
  {code:"03 / DIFFERENCE",title:lang==="zh"?"让信息茧房彼此相遇":"Bringing separate information bubbles into relation",body:editorial?editorial.essay[5]:c.essay[2]},
  {code:"04 / FORM",title:lang==="zh"?"最终形式由一百个人共同形成":"The final form will be made by one hundred people",body:editorial?editorial.essay[7]:c.essay[3]}
 ];
 return <main className={`publicPage ${view==="concept"?"conceptPage":"peoplePage"}`}><header className="top"><a className="publicBack" href="/">← {c.back}</a><a className="mark" href="/" aria-label="WHAT 100 PEOPLE DO TO A GAME"><Wordmark/></a><nav aria-label={c.publicPages}><a className={view==="concept"?"active":""} href="/concept" onClick={go("concept")}>{c.concept}</a><a className={view==="people"?"active":""} href="/people" onClick={go("people")}>{c.people}</a></nav><label className="languagePicker"><span><b aria-hidden="true">🌐</b>{c.language}</span><select aria-label={c.language} value={lang} onChange={e=>{const next=e.target.value as Language;storageSet("hundred-language",next);setLang(next)}}>{languageOptions.filter(x=>x.value==="zh"||x.value==="en").map(x=><option key={x.value} value={x.value}>{x.label}</option>)}</select></label></header>{view==="concept"?<section className="conceptStandalone"><header className="conceptHero" data-reveal><span className="kicker conceptLabelBlock" aria-label="WHAT 100 PEOPLE DO TO A GAME · 2026"><span className="conceptWord conceptWordWhat" aria-hidden="true">WHAT</span><span className="conceptWord conceptWordHundred" aria-hidden="true">100</span><span className="conceptWord conceptWordPeople" aria-hidden="true">PEOPLE</span><span className="conceptWord conceptWordConnector" aria-hidden="true">DO TO A</span><span className="conceptWord conceptWordGame" aria-hidden="true">GAME</span><span className="conceptWord conceptWordYear" aria-hidden="true">2026</span></span><h1><span className="conceptHeadlineLine">{editorial?editorial.headline[0]:c.headline[0]}</span><span className="conceptHeadlineLine accent">{editorial?editorial.headline[1]:c.headline[1]}</span></h1><p className="conceptSubtitle">{editorial?editorial.subtitle:c.subtitle}</p><div className="conceptMetrics" aria-label={lang==="zh"?"项目结构":"Project structure"}><span><b>100</b><small>{lang==="zh"?"个人":"PEOPLE"}</small></span><span><b>30</b><small>{lang==="zh"?"天":"DAYS"}</small></span><span><b>01</b><small>{lang==="zh"?"共同作品":"SHARED WORK"}</small></span></div></header><section className="conceptFeature" data-reveal><figure className="projectPoster"><a href="/project-recruitment-poster.jpg" target="_blank" rel="noreferrer"><img src="/project-recruitment-poster.jpg" width="1024" height="1536" loading="lazy" decoding="async" alt={lang==="zh"?"WHAT 100 PEOPLE DO TO A GAME 项目招募海报":"WHAT 100 PEOPLE DO TO A GAME recruitment poster"}/></a><figcaption>{lang==="zh"?"项目招募海报 · 2026":"Project recruitment poster · 2026"}</figcaption></figure><div className="conceptFeatureCopy"><span>PROJECT 01 / OPEN CALL</span><h2>{lang==="zh"?"我们不寻找一百名游戏设计师。":"We are not looking for one hundred game designers."}</h2><p><span>{editorial?editorial.essay[3]:c.essay[0]}</span></p><strong>{lang==="zh"?"我们寻找一百个人。":"We are looking for one hundred people."}</strong></div></section><section className="conceptManifesto" aria-label={lang==="zh"?"项目理念":"Project manifesto"}>{manifesto.map((item,index)=>{const expanded=!compactManifesto||openManifesto===index;return <article key={item.code} className={openManifesto===index?"is-open":""}><button type="button" className="manifestoToggle" aria-expanded={expanded} aria-controls={`manifesto-body-${index}`} aria-disabled={!compactManifesto} tabIndex={compactManifesto?0:-1} onClick={()=>{if(compactManifesto)setOpenManifesto(current=>current===index?-1:index)}}><span className="manifestoCode">{item.code}</span><h2>{item.title}</h2><i aria-hidden="true"/></button><div id={`manifesto-body-${index}`} className="manifestoBody"><div><p>{item.body}</p></div></div></article>})}</section>{editorial&&<><div className="conceptQuestions" data-reveal><span>QUESTIONS FOR THE MEDIUM</span>{editorial.questions.map(question=><p key={question}>{question}</p>)}</div><aside className="conceptRecruitment" data-reveal><span className="kicker">{editorial.call.kicker}</span><h2>{editorial.call.title}</h2><p>{editorial.call.body}</p><strong>{editorial.call.deadline}</strong><p>{editorial.call.contact}</p><b>{editorial.call.action}</b></aside></>}<a className="peopleCta" href="/people" onClick={go("people")} data-reveal>{c.meet} →</a></section>:<PublicDesignerRanking lang={lang}/>}<footer className="copyright">{t.copyright}</footer></main>
}
