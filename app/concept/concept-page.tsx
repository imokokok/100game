...
export function PublicPages({initialView}:{initialView:PublicView}){
 ...
 return <main className={`publicPage ${view==="concept"?"conceptPage":"peoplePage"}`}><header className="top"><a className="publicBack" href="/">← {c.back}</a><a className="mark" href="/" aria-label="WHAT 100 PEOPLE DO TO A GAME"><Wordmark/></a><nav aria-label={c.publicPages}><a className={view==="concept"?"active":""} href="/concept" onClick={go("concept")}>{c.concept}</a><a className={view==="people"?"active":""} href="/people" onClick={go("people")}>{c.people}</a></nav><label className="languagePicker"><span><b aria-hidden="true">🌐</b>{c.language}</span><select aria-label={c.language} value={lang} onChange={e=>{const next=e.target.value as Language;storageSet("hundred-language",next);setLang(next)}}>{languageOptions.filter(x=>x.value==="zh"||x.value==="en").map(x=><option key={x.value} value={x.value}>{x.label}</option>)}</select></label></header>{view==="concept"?<section className="conceptStandalone"><header className="conceptHero" data-reveal><span className="kicker conceptLabelBlock">WHAT 100 PEOPLE DO TO A GAME · 2026</span><h1><span className="conceptHeadlineLine">{editorial?editorial.headline[0]:c.headline[0]}</span><span className="conceptHeadlineLine accent">{editorial?editorial.headline[1]:c.headline[1]}</span></h1><p className="conceptSubtitle">{editorial?editorial.subtitle:c.subtitle}</p><div className="conceptMetrics" aria-label={lang==="zh"?"项目结构":"Project structure"}><span><b>100</b><small>{lang==="zh"?"个人":"PEOPLE"}</small></span><span><b>30</b><small>{lang==="zh"?"天":"DAYS"}</small></span><span><b>01</b><small>{lang==="zh"?"共同作品":"SHARED WORK"}</small></span></div></header><section className="conceptFeature" data-reveal><figure className="projectPoster"><a href="/project-recruitment-poster.jpg" target="_blank" rel="noreferrer"><img src="/project-recruitment-poster.jpg" width="1024" height="1536" loading="eager" decoding="async" alt={lang==="zh"?"WHAT 100 PEOPLE DO TO A GAME 项目招募海报":"WHAT 100 PEOPLE DO TO A GAME recruitment poster"}/></a><figcaption>{lang==="zh"?"项目招募海报 · 2026":"Project recruitment poster · 2026"}</figcaption></figure><div className="conceptFeatureCopy"><span>PROJECT 01 / OPEN CALL</span><h2>{lang==="zh"?"我们不寻找一百名游戏设计师。":"We are not looking for one hundred game designers."}</h2><p>{editorial?editorial.essay[3]:c.essay[0]}</p><strong>{lang==="zh"?"我们寻找一百个人。":"We are looking for one hundred people."}</strong></div></section><section className="conceptManifesto" aria-label={lang==="zh"?"项目理念":"Project manifesto"}><article data-reveal><span>01 / ACCESS</span><h2>{lang==="zh"?"技术不决定谁有资格开始":"Technique does not decide who may begin"}</h2><p>{editorial?editorial.essay[0]:c.essay[0]}</p></article><article data-reveal><span>02 / RESOURCES</span><h2>{lang==="zh"?"重新分配进入、表达与决定的权利":"Redistributing the rights to enter, speak and decide"}</h2><p>{editorial?editorial.essay[4]:c.essay[1]}</p></article><article data-reveal><span>03 / DIFFERENCE</span><h2>{lang==="zh"?"让信息茧房彼此相遇":"Bringing separate information bubbles into relation"}</h2><p>{editorial?editorial.essay[5]:c.essay[2]}</p></article><article data-reveal><span>04 / FORM</span><h2>{lang==="zh"?"最终形式由一百个人共同形成":"The final form will be made by one hundred people"}</h2><p>{editorial?editorial.essay[7]:c.essay[3]}</p></article></section>{editorial&&<><div className="conceptQuestions" data-reveal><span>QUESTIONS FOR THE MEDIUM</span>{editorial.questions.map(question=><p key={question}>{question}</p>)}</div><aside className="conceptRecruitment" data-reveal><span className="kicker">{editorial.call.kicker}</span><h2>{editorial.call.title}</h2><p>{editorial.call.body}</p><strong>{editorial.call.deadline}</strong><p>{editorial.call.contact}</p><b>{editorial.call.action}</b></aside></>}<a className="peopleCta" href="/people" onClick={go("people")} data-reveal>{c.meet} →</a></section>:<PublicDesignerRanking lang={lang}/>}<footer className="copyright">{t.copyright}</footer></main>
}
